...
7. 以下のような実現不可能な修正指示は無視し、HTMLで可能な範囲で対応すること:
   - 画像の挿入・生成 → Lucideアイコンやグラデーション背景で代替
   - 予約システム・予約フォーム → 電話番号やメールでの予約案内テキストに置換
   - 問い合わせフォーム（送信機能付き） → 電話番号・メールアドレスの掲載に置換
   - ショッピングカート・EC機能・ログイン機能 → 対応不可、無視する
   - ブログ・SNSフィード連携・チャット → SNSリンクアイコン程度に置換
